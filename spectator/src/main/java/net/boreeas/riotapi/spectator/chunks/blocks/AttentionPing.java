...
public class AttentionPing extends Block {
    private float x;
    private float y;
    private long targetEntity;
    private long pingingEntity;
    private PingType pingType;

    public AttentionPing(BlockHeader header, byte[] data) {
        super(header, data);

        ByteBuffer buffer = ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN);
        this.x = buffer.getFloat();
        this.y = buffer.getFloat();
        this.targetEntity = buffer.getInt() & 0xffffffffL;
        this.pingingEntity = buffer.getInt() & 0xffffffffL;
        this.pingType = PingType.getBySpectatorId(buffer.get() & 0xff);
