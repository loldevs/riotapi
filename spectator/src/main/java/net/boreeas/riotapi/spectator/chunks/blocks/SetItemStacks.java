...
@Value
@Log4j
@IsBlock(BlockType.SET_ITEM_STACKS)
public class SetItemStacks extends Block {
    private long entityId;
    private int slotId;
    private int countOnSlot;

    public SetItemStacks(BlockHeader header, byte[] data) {
        super(header, data);

        this.entityId = header.getBlockOwner();

        ByteBuffer buffer = ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN);
        this.slotId = buffer.get() & 0xff;
        this.countOnSlot = buffer.getShort() & 0xffff;
